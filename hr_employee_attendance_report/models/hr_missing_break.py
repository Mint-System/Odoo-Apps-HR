...
class HrMissingBreak(models.Model):
    ...
    deducted_on = fields.Datetime(string="Deducted On", readonly=True)
    ...
    def action_deduct_missing_break(self):
        ...
        if not records:
            raise UserError(
                _("No pending missing breaks to deduct. " "All selected breaks have already been processed.")
            )
        ...
        for break_rec in records:
            ...
            day_date = fields.Date.context_today(self, timestamp=break_rec.working_day)
            ...
            if overtime:
                ...
                _logger.info(
                    "Missing break deducted: employee=%s day=%s " "overtime %.2f → %.2f h",
                    employee.name,
                    day_date,
                    overtime.duration + BREAK_DURATION_HOURS,
                    new_duration,
                )
            else:
                ...
                _logger.info(
                    "Missing break deducted (no prior OT): employee=%s day=%s " "→ -%.2f h overtime created",
                    employee.name,
                    day_date,
        ...
        return {
            ...
            "params": {
                ...
                "message": _("%d missing break(s) of 30 min have been deducted from overtime.") % deducted_count,
                "type": "success",
                "sticky": False,
            },
        }
