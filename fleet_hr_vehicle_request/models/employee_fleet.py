...
class EmployeeFleet(models.Model):
    ...
    trip = fields.Float(copy=False, readonly=True, states={'confirm': [('readonly', False)]})
    state = fields.Selection(tracking=True)
    odometer = fields.Float(string='Last Odometer Value', copy=False, readonly=True, states={'confirm': [('readonly', False)]})
    private_usage = fields.Boolean(readonly=True, states={'draft': [('readonly', False)]})
    ...
    def _onchange_trip(self):
        ...
        last_odometer = self.env['fleet.vehicle.odometer'].search([('vehicle_id','=',self.fleet.id)], order="value desc", limit=1)
        if last_odometer:
