...
class Applicant(models.Model):
    ...
    def _compute_name(self):
        for record in self:
            record.partner_name = self.env['res.partner']._get_computed_name(record.lastname, record.firstname)
    
    def website_form_input_filter(self, request, values):
        if 'firstname' in values and 'lastname' in values:
            values.setdefault('name', '%s %s\'s Application' % (values['firstname'], values['lastname']))
        return values
