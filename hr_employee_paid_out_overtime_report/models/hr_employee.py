...
class HrEmployee(models.Model):
    ...
    total_paid_out_overtime = fields.Float(
        compute='_compute_total_paid_out_overtime', compute_sudo=True,
    ...
    def _compute_total_paid_out_overtime(self):
        for employee in self:
            if employee.company_id.hr_attendance_overtime:
                employee.total_paid_out_overtime = round(sum(employee.overtime_ids.filtered(lambda overtime: overtime.paid_out).mapped('duration')), 2)
            else:
