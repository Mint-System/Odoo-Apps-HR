...
class HrEmployee(models.Model):
    ...
    total_overtime_paid_out = fields.Float(
        compute='_compute_total_overtime_paid_out', compute_sudo=True,
    ...
    def _compute_total_overtime_paid_out(self):
        for employee in self:
            if employee.company_id.hr_attendance_overtime:
                employee.total_overtime_paid_out = float_round(sum(employee.overtime_ids.filtered(lambda o: o.paid_out).mapped('duration')), 2)
            else:
