...
class EmployeeFleet(models.Model):
    ...
    date_to = fields.Datetime(string="To", required=True, readonly=True, states={"draft": [("readonly", False)]})
    ...
    def send(self):
        if self.vehicle_id not in self.available_vehicle_ids:
            ...
            manager = self.vehicle_id.manager_id
            partner_ids = [manager.partner_id.id] if manager and manager.partner_id else []

            self.message_post(
                subject=_("%s: Validation") % self.name,
                body=_("Hi %s,<br>You have received the vehicle request %s for validation.")
                % (manager.name or "", self.name),
    ...
    def reject(self):
        ...
        self.message_post(
            ...
            body=_("Hi %s,<br>Sorry, Your vehicle request for the reference %s is rejected.")
            % (self.employee.name, self.name),
            partner_ids=[self.employee.user_partner_id.id],
        )
