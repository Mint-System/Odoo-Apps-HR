...
class HrAttendance(models.Model):
    ...
    check_out_display = fields.Char(string="Check Out", compute="_compute_check_out_display", store=False)
    ...
    def create_or_delete_missing_break(self):
        ...
        if check_out - check_in > timedelta(hours=7):
            new_break = self.env["hr.missing.break"].create(
                {"employee_id": employee.id, "working_day": missing_break_date}
            )
    ...
    def calculate_missing_break(self):
        leave_missing_break = self.env["hr.leave.type"].search(
            [("code", "=", "MB"), ("company_id", "=", self.env.company.id)]
        )
        if not leave_missing_break:
            leave_missing_break = self.env["hr.leave.type"].create(
                {
                    "name": "Missing Break",
                    "code": "MB",
                ...
                self.env["hr.leave"].create(
                    {
                        ...
                        "holiday_status_id": leave_missing_break.id,
                        "date_from": break_start_date,
                        "date_to": break_end_date,
                    }
                )
    ...
    def deduct_or_add_missing_break(self, mbreak, action):
        ...
        day_date = fields.Date.context_today(self, timestamp=mbreak.working_day)
