...
class HrAttendance(models.Model):
    ...
    def _cron_absence_detection(self):
        ...
        checked_in_employees = (
            ...
            .search([("date", "=", yesterday), ("adjustment", "=", False)])
            .employee_id
        )
        ...
        absent_employees = absent_employees.filtered(
            lambda e: e.user_id
            and e.user_id.has_group("hr_attendance_user_acl.group_hr_attendance_user")
        )

        for emp in absent_employees:
            local_day_start = pytz.utc.localize(yesterday).astimezone(
